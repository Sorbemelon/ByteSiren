...
async function runBrowserSmoke() {
  ...
  try {
    ...
    await waitForCondition(
      session,
      `document.querySelector('[data-testid="feed-section-toggle-v02"][data-section-id="sig_fixture"]')?.textContent.includes('Hide') && document.body.innerText.includes('Window Change') && document.body.innerText.includes('Peak 15m') && document.body.innerText.includes('Volume ×') && document.body.innerText.includes('Range Position')`,
    );
