...
const SIGNAL_EVENT_SYSTEM_PROMPT = [
  ...
  "- A source article may be published after the Signal Event only if it clearly reports a catalyst that happened inside the allowed catalyst window; in that case catalyst_time_utc must be inside the allowed catalyst window.",
  ...
  "- Backdrop sources for a Signal Event must still be relevant to the event's UTC day; broad multi-day-old macro context belongs in Daily Overview, not this Signal Event.",
  "- Later recaps without an in-window catalyst_time_utc should be omitted or rejected; they cannot be Focused/Likely.",
  ...
  "- If no returned source supports a news/context claim, collapsed_summary must say no time-aligned public source was accepted and summarize only the Signal Event evidence.",
  "",
  "Brief rules:",
