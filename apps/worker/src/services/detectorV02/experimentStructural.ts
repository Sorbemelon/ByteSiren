...
import { SYMBOLS, median, roundNumber } from "./experimentShared.ts";
import {
...
function symbolShapeAt({ symbol, candles, indicators, times, c, options }) {
  ...
  const ctx = computeSymbolChartContext({
    symbol,
    event,
    candles,
    indicators,
  ...
  if (
    ...
    if (ctx.squeeze_break_flag) {
      return {
        pattern: "compression_expansion_break",
        direction: ctx.range_break_direction,
      };
    }
...
export function detectStructuralEvents({
  ...
  const candidates = windows.map((window) => {
    ...
    candidate.structural_pattern_components =
      window.structural_pattern_components;
    ...
    candidate.direction_history = (window.direction_history ?? []).map(
      (step) => ({
    ...
    candidate.event_story_type =
      STRUCTURAL_STORY[window.structural_pattern](direction);
    return candidate;
  });
...
export function mergePublicSignals(
  ...
  for (const event of publicEvents) {
    ...
    if (sinceLastMin <= gapMin && withinCap) {
      ...
      if (event.window_end > group.runningEnd)
        group.runningEnd = event.window_end;
    } else {
      groups.push(group);
      group = { members: [event], runningEnd: event.window_end };
    }
