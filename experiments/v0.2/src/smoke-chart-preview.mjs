...
// Default smoke for the accepted structural chart-preview bundle
// (chart-preview/data). Validates structure + structural-detector expectations.
...
const DATA_DIR = path.join(CHART_PREVIEW_DIR, "data");
...
async function loadBundle() {
  ...
  assert.ok(
    ...
    `missing default structural bundle: ${generatedPath} (run build-structural-preview.mjs)`,
  );
...
async function run() {
  ...
  assert.ok(
    previewHtml.includes("./data/preview-data.generated.js"),
    "index.html must load the default structural data bundle",
  );
